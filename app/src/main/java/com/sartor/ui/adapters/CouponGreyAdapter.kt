...
class CouponGreyAdapter(val list: List<Coupon>, val context: Context) : RecyclerView.Adapter<CouponGreyAdapter.ViewHolder>(){

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(
            LayoutInflater.from(parent.context).inflate(
                R.layout.child_coupon_grey, parent, false
            )
        )
    }
